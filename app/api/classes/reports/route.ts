...
export async function GET(request: NextRequest) {
  try {
    ...
    const summary = {
      ...
      totalStudents: instructorReports.reduce((sum, r) => sum + r.totalStudents, 0),
      totalRevenue: instructorReports.reduce((sum, r) => sum + r.totalRevenue, 0),
      collectedRevenue: instructorReports.reduce((sum, r) => sum + r.collectedRevenue, 0),
      pendingRevenue: instructorReports.reduce((sum, r) => sum + r.pendingRevenue, 0),
      overallAttendanceRate: 0,
      overallCollectionRate: 0
    }
    
    if (summary.totalStudents > 0) {
      const totalAttended = instructorReports.reduce((sum, r) => sum + r.totalAttended + r.totalLate, 0)

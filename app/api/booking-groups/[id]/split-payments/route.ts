...
export async function GET(
  ...
  try {
    ...
    const status = {
      ...
      completedPayments: bookingGroup.splitPayments.filter(sp => sp.status === 'completed').length,
      pendingPayments: bookingGroup.splitPayments.filter(sp => sp.status === 'pending').length,
      totalPaid: bookingGroup.splitPayments
        .filter(sp => sp.status === 'completed')
        .reduce((sum, sp) => sum + sp.amount, 0),
      isComplete: bookingGroup.splitPayments.every(sp => sp.status === 'completed')
    }
...
export async function PUT(
  ...
  try {
    ...
    if (action === 'complete') {
      ...
      console.log(`💰 Group transaction created for split payment: ${splitPayment.playerName} - $${(splitPayment.amount / 100).toFixed(2)} MXN`)
    ...
    return NextResponse.json({
      success: false,
      error: 'Regeneración de payment intent no implementada para grupos'
    }, { status: 501 })

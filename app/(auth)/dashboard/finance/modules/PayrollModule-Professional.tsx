...
export default function PayrollModuleProfessional() {
  console.log('🎯 PayrollModule Component Mounted!')
  ...
  useEffect(() => {
    console.log('🎯 PayrollModule useEffect triggered')
    fetchPayrollData()
  }, [selectedPeriod])
  ...
  const fetchPayrollData = async () => {
    try {
      ...
      console.log('🔍 [FETCH START] Period:', period)
      console.log('🔍 [FETCH] URL:', `/api/finance/payroll?period=${period}`)
      ...
      if (response.ok) {
        ...
        console.log('🔍 [DATA] Payroll array:', data.payroll)
        console.log('🔍 [DATA] Number of records:', data.payroll?.length || 0)
        console.log('🔍 [DATA] First record:', data.payroll?.[0])
        ...
        // Map payroll records to unique employees (deduplicate by name)
        const employeeMap = new Map<string, Employee>()
        
        data.payroll?.forEach((r: any) => {
          if (!employeeMap.has(r.employeeName)) {
            employeeMap.set(r.employeeName, {
              id: r.id,
              name: r.employeeName,
              role: r.employeeRole || 'Empleado',
              type: r.employeeRole?.toLowerCase().includes('instructor') || 
                    r.employeeRole?.toLowerCase().includes('entrenador') || 
                    r.employeeRole?.toLowerCase().includes('coach') ? 'instructor' : 'fixed',
              baseSalary: r.netAmount || r.baseSalary || 0, // Use netAmount for display
              status: 'active',
              startDate: r.createdAt || new Date().toISOString(),
              department: r.employeeRole?.includes('Admin') ? 'Administración' : 
                         r.employeeRole?.includes('Manten') ? 'Mantenimiento' : 
                         r.employeeRole?.includes('Instructor') ? 'Deportivo' : 'General',
              classes: r.employeeRole?.toLowerCase().includes('instructor') ? Math.floor(Math.random() * 20) + 5 : undefined,
              students: r.employeeRole?.toLowerCase().includes('instructor') ? Math.floor(Math.random() * 40) + 10 : undefined
            })
          }
        })
        
        const mappedEmployees: Employee[] = Array.from(employeeMap.values())
        console.log('🔍 [MAPPING] Unique employees:', mappedEmployees.length)
        console.log('🔍 [MAPPING] Employee list:', mappedEmployees)
        
        const mappedRecords: PayrollRecord[] = data.payroll?.map((r: any) => ({
          id: r.id,
          employeeId: r.id,
          employeeName: r.employeeName,
          period: r.period,
          baseSalary: r.baseSalary || 0, // Keep in cents
          bonuses: r.bonuses || 0, // Keep in cents
          deductions: r.deductions || 0, // Keep in cents
          netAmount: r.netAmount || 0, // Keep in cents
          status: r.status || 'pending',
          paymentDate: r.paidAt
        })) || []
        
        console.log('🔍 [SETTING] About to set employees:', mappedEmployees)
        console.log('🔍 [SETTING] About to set records:', mappedRecords)
        
        setEmployees(mappedEmployees)
        setPayrollRecords(mappedRecords)
        
        console.log('🔍 [STATE] Employees set:', mappedEmployees.length)
        console.log('🔍 [STATE] Records set:', mappedRecords.length)
      } else {
        console.error('📊 Failed to fetch payroll data:', response.status, response.statusText)
  ...
  const handleCreatePayroll = async (employeeName: string, employeeRole: string, baseSalary: number, bonuses: number = 0, deductions: number = 0) => {
    try {
      setProcessing(true)
      const period = format(selectedPeriod, 'yyyy-MM')
      ...
      const response = await fetch('/api/finance/payroll', {
        ...
        body: JSON.stringify({
          ...
          bonuses,
          deductions,
          notes: `Nómina del mes ${format(selectedPeriod, 'MMMM yyyy', { locale: es })}`
        })
      })
      ...
      if (response.ok) {
        ...
        alert(error.error || 'Error al crear registro de nómina')
      }
    } catch (error) {
      console.error('Error creating payroll:', error)
      alert('Error al crear registro de nómina')
    } finally {
      setProcessing(false)
    }
  }

  const handleUpdatePayrollStatus = async (payrollId: string, status: 'paid' | 'cancelled') => {
    try {
      const response = await fetch('/api/finance/payroll', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok) {
        ...
        alert(error.error || 'Error al actualizar nómina')
      }
    } catch (error) {
      console.error('Error updating payroll:', error)
      alert('Error al actualizar nómina')
    }
  }
  ...
  const handleProcessPayroll = async () => {
    try {
      ...
      // Get all pending payrolls for the period and mark them as paid
      const pendingPayrolls = payrollRecords.filter(r => r.status === 'pending' && r.period === period)
      
      for (const payroll of pendingPayrolls) {
        await handleUpdatePayrollStatus(payroll.id, 'paid')
      }
      
      alert(`Se procesaron ${pendingPayrolls.length} pagos de nómina exitosamente`)
    } catch (error) {
  ...
  console.log('🎨 [RENDER] Current employees state:', employees)
  console.log('🎨 [RENDER] Current payrollRecords state:', payrollRecords)
  
  const fixedEmployees = employees.filter(e => e.type === 'fixed')
  const instructors = employees.filter(e => e.type === 'instructor')
  
  console.log('🎨 [RENDER] Fixed employees:', fixedEmployees.length, fixedEmployees)
  console.log('🎨 [RENDER] Instructors:', instructors.length, instructors)
  
  // Calculate totals from payroll records for the selected period
  const currentPeriodRecords = payrollRecords.filter(r => r.period === format(selectedPeriod, 'yyyy-MM'))
  const totalPayroll = currentPeriodRecords.reduce((sum, r) => sum + r.netAmount, 0)
  const totalEmployees = employees.length
  const activeEmployees = employees.filter(e => e.status === 'active').length
  const pendingPayments = currentPeriodRecords.filter(r => r.status === 'pending').length
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        <p style={{
                          ...
                          {formatCurrency(instructor.baseSalary)}
                        </p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                {payrollRecords.length > 0 ? (
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                          <div style={{
                            ...
                            height: '40px',
                            borderRadius: '10px',
                            background: record.status === 'pending' ? 'rgba(251, 191, 36, 0.1)' : 'rgba(164, 223, 78, 0.1)',
                            display: 'flex',
                            alignItems: 'center',
                            justifyContent: 'center'
                          }}>
                            <CheckCircle style={{ 
                              width: '20px', 
                              height: '20px', 
                              color: record.status === 'pending' ? '#FBBF24' : '#A4DF4E' 
                            }} />
                          </div>
                          <div>
                            <p style={{ 
                            ...
                            <p style={{ 
                              ...
                              {record.paymentDate ? `Pagado el ${format(new Date(record.paymentDate), 'dd MMM yyyy', { locale: es })}` : 'Pendiente de pago'}
                            </p>
                          </div>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                          <div style={{ textAlign: 'right' }}>
                            ...
                            <span style={{
                              ...
                              {record.status === 'pending' ? 'Pendiente' : 'Pagado'}
                            </span>
                          </div>
                          <div style={{ display: 'flex', gap: '4px' }}>
                            <button
                  ...
                  <div style={{ 
                    ...
                    {loading ? 'Cargando registros de nómina...' : 'No hay registros de nómina para mostrar'}
                  </div>
                )}
              </div>
            )}
            ...
            <div style={{ padding: '24px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '14px', color: '#516640' }}>Empleados</span>
                  <span style={{ fontSize: '14px', fontWeight: 600, color: '#3B82F6' }}>
                    {formatCurrency(fixedEmployees.reduce((sum, e) => sum + e.baseSalary, 0))}
                  </span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '14px', color: '#516640' }}>Instructores</span>
                  <span style={{ fontSize: '14px', fontWeight: 600, color: '#9333EA' }}>
                    {formatCurrency(instructors.reduce((sum, i) => sum + i.baseSalary, 0))}
                  </span>
                </div>
                <div style={{
            ...
            <div style={{ padding: '24px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <button
                  ...
                  disabled={processing || pendingPayments === 0}
                  style={{
                    width: '100%',
                    padding: '12px 16px',
                    borderRadius: '10px',
                    ...
                    fontSize: '14px',
                    fontWeight: 500,
                    cursor: processing || pendingPayments === 0 ? 'not-allowed' : 'pointer',
                    display: 'flex',
                    alignItems: 'center',
                    gap: '8px',
                    transition: 'all 0.2s',
                    opacity: processing || pendingPayments === 0 ? 0.5 : 1
                  }}
                  onMouseEnter={(e) => {
                    if (!processing && pendingPayments > 0) e.currentTarget.style.background = 'rgba(164, 223, 78, 0.05)'
                  }}
                  onMouseLeave={(e) => {
                ...
                >
                  ...
                  {processing ? 'Procesando...' : `Procesar Nómina (${pendingPayments})`}
                </button>
                <button
                  style={{
                    width: '100%',
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {(() => {
                  ...
                  const departmentBreakdown = employees.reduce((acc, emp) => {
                    ...
                    if (!acc[dept]) {
                      acc[dept] = { employees: 0, amount: 0 }
                    }
                    acc[dept].employees += 1
                    acc[dept].amount += emp.baseSalary // baseSalary is already the total amount
                    return acc
                  }, {} as Record<string, { employees: number; amount: number }>)

                  const colors = ['#3B82F6', '#EF4444', '#9333EA', '#10B981', '#F59E0B']
                  const departments = Object.entries(departmentBreakdown).map(([name, data], index) => ({
                    name,
                    ...data,
                  ...
                  return departments.map((dept, index) => (
                    <div key={dept.name}>
                      ...
                      <div style={{
                        ...
                        <div style={{
                          width: `${totalPayroll > 0 ? (dept.amount / totalPayroll) * 100 : 0}%`,
                          height: '100%',

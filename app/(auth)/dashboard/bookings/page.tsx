...
import { getNowInTimezone } from '@/lib/utils/timezone'
...
function BookingsPageContent() {
  ...
  const [clubTimezone, setClubTimezone] = useState('America/Mexico_City') // Default fallback
  // Set initial date to today in club timezone - force current date
  const [selectedDate, setSelectedDate] = useState(() => {
    // FIXED: Force current date without any timezone confusion
    const today = new Date()
    console.log('✅ FIXED - Initial selectedDate set to TODAY:', today, 'Month:', today.getMonth() + 1)
    return today
  })
  ...
  // No more complex timezone effects - keep it simple
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (data.success && data.settings) {
        ...
        // Update club timezone (this will trigger the useEffect to update selectedDate)
        const timezone = data.settings.timezone || 'America/Mexico_City'
        console.log('Updating clubTimezone to:', timezone)
  ...
  const fetchBookings = async () => {
    setIsLoading(true)
    try {
      // 🔥 FORCE CURRENT DATE - OVERRIDE EVERYTHING
      const TODAY = new Date()
      console.log('🔥 FORCE OVERRIDE - Using TODAY instead of selectedDate - RECOMPILED!')
      console.log('🔥 TODAY:', TODAY, 'Month:', TODAY.getMonth() + 1)
      console.log('🔥 selectedDate was:', selectedDate, 'Month:', selectedDate.getMonth() + 1)
            ...
            <div style={{ padding: '24px' }}>
              <div style={{
                ...
                <h3 style={{
                  ...
                  {(() => {
                    ...
                    console.log('🔍 Rendering month - selectedDate:', selectedDate, 'monthIndex:', monthIndex, 'monthName:', monthName)
                    
                    // DEBUG: Show exact date being rendered
                    if (typeof window !== 'undefined') {
                      console.log('🚨 BROWSER RENDER - Month index:', monthIndex, 'Month name:', monthName, 'Full date:', selectedDate.toString())
                    }
                ...
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  {/* DEBUG: Add button to force current date */}
                  <button
                    onClick={() => {
                      const now = getNowInTimezone(clubTimezone)
                      console.log('🔧 DEBUG: Forcing current date:', now, 'month:', now.getMonth())
                      setSelectedDate(now)
                      alert(`DEBUG: Set date to ${now.toString()} (month: ${now.getMonth() + 1})`)
                    }}
                    style={{
                      padding: '4px 8px',
                      fontSize: '12px',
              ...
              <div style={{
                ...
                {(() => {
                  ...
                  const today = getNowInTimezone(clubTimezone)
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    {formatCurrency(filteredBookings.reduce((sum, b) => sum + (b.price || 0), 0) / 100)}
                  </div>
                  <div style={{
                    fontSize: '12px',
                    color: '#516640',
                      ...
                      >
                        ...
                        <div style={{
                          ...
                          <div style={{
                            ...
                            {booking.isGroup ? booking.playerName : booking.playerName}
                          </div>
                          ...
                          <div onClick={(e) => e.stopPropagation()}>
                            {booking.checkedIn ? (
                              ...
                              <ButtonModern
                                ...
                                style={{
                                  background: (booking.paymentStatus === 'paid' || booking.paymentStatus === 'completed')
                                    ? 'linear-gradient(135deg, #A4DF4E, #66E7AA)'
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                    ...
                    <CardModernDescription>
                      {selectedBooking.isGroup ? selectedBooking.playerName : selectedBooking.playerName}
                    </CardModernDescription>
                  </div>
                  <ButtonModern
                    variant="ghost"

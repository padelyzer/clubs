...
export default function TournamentsIncomeModule() {
  ...
  const calculateStats = (tournaments: TournamentTransaction[]) => {
    ...
    const totalRevenue = filtered.reduce((sum, t) => sum + t.totalRevenue, 0)
    const totalPrizeMoney = filtered.reduce((sum, t) => sum + t.prizeMoney, 0)
    const totalParticipants = filtered.reduce((sum, t) => sum + t.participants, 0)

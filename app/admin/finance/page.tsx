...
export default async function AdminFinancePage() {
  ...
  const stats = {
    revenue: {
      ...
      year: (yearRevenue._sum.amount || 0) / 100,
      growth: lastMonthRevenue._sum.amount ? 
        (((monthRevenue._sum.amount || 0) - (lastMonthRevenue._sum.amount || 0)) / (lastMonthRevenue._sum.amount || 1) * 100) : 0
    },
    commission: {
      total: ((totalCommission._sum.amount || 0) * 0.025) / 100,
      month: ((monthCommission._sum.amount || 0) * 0.025) / 100,
      estimated: ((monthRevenue._sum.amount || 0) * 0.025) / 100 // 2.5% promedio
    },
    bookings: {
  ...
  const trends = monthlyTrends.map((month, index) => {
    ...
    return {
      ...
      revenue: (month._sum.amount || 0) / 100,
      bookings: month._count,
      commission: ((month._sum.amount || 0) * 0.025) / 100
    }
  })
  ...
  const clubRevenueData = clubRevenues.map(club => ({
    id: club.id,
    name: club.name,
    revenue: club.Booking.reduce((sum, booking) => sum + booking.amount, 0) / 100,
    bookings: club.Booking.length,
    commission: (club.Booking.reduce((sum, booking) => sum + booking.amount, 0) * 0.025) / 100
  })).sort((a, b) => b.revenue - a.revenue)

  // Procesar pagos pendientes
  const pendingPayoutsData = pendingPayouts.map(club => {
    const clubRevenue = club.Booking.reduce((sum, booking) => sum + booking.amount, 0) / 100

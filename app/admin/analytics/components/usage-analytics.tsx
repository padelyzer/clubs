...
export default function UsageAnalytics({ usage, revenueAnalysis }: UsageAnalyticsProps) {
  ...
  const peakHours = Array.from({ length: 24 }, (_, hour) => {
    const baseBookings = usage.dailyBookings.reduce((sum, day) => sum + day, 0) / 30
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-2xl font-bold">
                {Math.round(usage.dailyBookings.reduce((sum, day) => sum + day, 0) / 30)}
              </div>
              <div className="text-sm opacity-90">Reservas/Día Promedio</div>
            </div>
            <div className="text-right">
              <div className="text-lg font-semibold">
                {usage.dailyBookings.reduce((sum, day) => sum + day, 0)}
              </div>
        ...
        <div className="bg-gradient-to-r from-amber-500 to-amber-600 text-white rounded-xl p-6">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <div className="text-lg font-semibold">
                {formatCurrency(usage.topClubs.reduce((sum, club) => sum + club.monthlyRevenue, 0))}
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center py-2">
              ...
              <span className="font-semibold text-blue-600">
                {usage.totalClubs > 0 
                  ? Math.round(usage.topClubs.reduce((sum, club) => sum + club.usersCount, 0) / usage.totalClubs)
                  : 0
                }
              </span>
            </div>

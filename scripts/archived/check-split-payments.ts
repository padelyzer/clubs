...
async function checkSplitPayments() {
  try {
    ...
    if (booking.splitPayment && booking.SplitPayment.length > 0) {
      console.log(`\n💳 Pagos Divididos (${booking.SplitPayment.length}/${booking.splitCount || 0}):`)
      ...
      console.log(`   Progreso: ${booking.SplitPayment.filter(sp => sp.status === 'completed').length}/${booking.splitCount || booking.SplitPayment.length} pagos completados`)

      // Verificar si faltan pagos divididos por crear
      if (booking.splitCount && booking.SplitPayment.length < booking.splitCount) {
        console.log(`\n⚠️  ADVERTENCIA: Faltan ${booking.splitCount - booking.SplitPayment.length} pagos divididos por crear`)
      }

      // Verificar pagos sin Payment Intent
      const paymentsWithoutIntent = booking.SplitPayment.filter(sp =>

  ...
  try {
    ...
    for (const booking of bookingsWithSplits) {
      ...
      if (booking.SplitPayment.length > 0) {
        ...
        for (const sp of booking.SplitPayment) {
      ...
      if (booking.splitPaymentCount && booking.SplitPayment.length < booking.splitPaymentCount) {
        console.log(`\n   ⚠️  PROBLEMA: Faltan ${booking.splitPaymentCount - booking.SplitPayment.length} pagos por crear`)
      }
    }
    ...
    if (anaBooking) {
      ...
      console.log(`   Split Payments: ${anaBooking.SplitPayment.length}`)

      if (anaBooking.SplitPayment.length === 0 && anaBooking.splitPaymentEnabled) {

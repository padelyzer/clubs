...
async function testSplitPaymentFlow() {
  try {
    ...
    console.log(`   Split Payments: ${booking.SplitPayment.length}`)
    ...
    const processing = booking.SplitPayment.filter(sp => sp.status === 'processing').length
    const pending = booking.SplitPayment.filter(sp => sp.status === 'pending').length

    console.log(`\n📊 Resumen:`)
    console.log(`   ✅ Completados: ${completed}`)
    console.log(`   🔄 En procesamiento: ${processing}`)
    console.log(`   ⏳ Pendientes: ${pending}`)
    console.log(`   📈 Progreso: ${completed}/${booking.SplitPayment.length}`)

    // 4. Verificar si hay pagos con Payment Intent que pueden probarse
    const paymentsWithIntent = booking.SplitPayment.filter(sp => sp.stripePaymentIntentId)
